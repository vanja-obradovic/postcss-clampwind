  ...
  return {
    ...
    prepare() {
      return {
        // MARK: Rule
        Rule(rule2) {
          if (rule2.selector === ":root") {
            rule2.walkDecls((decl2) => {
              if (decl2.prop.startsWith("--breakpoint-")) {
                const key = decl2.prop.replace("--breakpoint-", "");
                rootElementBreakpoints[key] = decl2.value;
              }
              if (decl2.prop.startsWith("--container-")) {
                const key = decl2.prop.replace("--container-", "@");
                rootElementContainerBreakpoints[key] = decl2.value;
              }
              if (decl2.prop === "--text-base" && decl2.value.includes("px")) {
                rootFontSize = parseFloat(decl2.value);
              }
              if (decl2.prop === "font-size" && decl2.value.includes("px")) {
                rootFontSize = parseFloat(decl2.value);
              }
              if (decl2.prop === "--spacing") {
                spacingSize = parseFloat(convertToRem(decl2.value, rootFontSize, spacingSize, customProperties));
              }
              if (decl2.prop.startsWith("--")) {
                const value = parseFloat(convertToRem(decl2.value, rootFontSize, spacingSize, customProperties));
                if (value) customProperties[decl2.prop] = value;
              }
            });
            return;
          }
          ...
          const hasMediaChild = (rule2.nodes || []).some(
            (n) => n.type === "atrule" && n.name === "media"
          );
          if (declNodes.length && !hasMediaChild) {
            noMediaQueries.push({ ruleNode: rule2, declNodes });
          }
        },
        // MARK: AtRule
        AtRule: {
          ...
          layer(atRule2) {
            if (atRule2.params === "default" && !Object.keys(defaultLayerBreakpoints).length) {
              const css = atRule2.source.input.css;
              const matches = css.match(/--breakpoint-[^:]+:\s*[^;]+/g) || [];
              defaultLayerBreakpoints = formatBreakpointsRegexMatches(matches);
            }
            if (atRule2.params === "default" && !Object.keys(defaultLayerContainerBreakpoints).length) {
              const css = atRule2.source.input.css;
              const matches = css.match(/--container-[^:]+:\s*[^;]+/g) || [];
              defaultLayerContainerBreakpoints = formatContainerBreakpointsRegexMatches(matches);
            }
            if (atRule2.params === "theme") {
              atRule2.walkDecls((decl2) => {
                if (decl2.prop.startsWith("--breakpoint-")) {
                  const key = decl2.prop.replace("--breakpoint-", "");
                  themeLayerBreakpoints[key] = decl2.value;
                }
                if (decl2.prop.startsWith("--container-")) {
                  const key = decl2.prop.replace("--container-", "@");
                  themeLayerContainerBreakpoints[key] = decl2.value;
                }
                if (decl2.prop === "--text-base" && decl2.value.includes("px")) {
                  rootFontSize = parseFloat(decl2.value);
                }
          ...
          media(atRule2) {
            const isNested = atRule2.parent?.type === "atrule";
            const isSameAtRule = atRule2.parent?.name === atRule2.name;
            const declNodes = [];
            for (const node of atRule2.nodes || []) {
              if (node.type === "decl" && extractTwoValidClampArgs(node.value)) {
                declNodes.push(node);
              }
            }
            ...
            if (isNested && isSameAtRule) {
              doubleNestedMediaQueries.push({
                parentNode: atRule2.parent,
                mediaNode: atRule2,
                declNodes
              });
            } else if (isNested && !isSameAtRule) {
              declNodes.forEach((decl2) => {
                decl2.value = ` ${decl2.value} /* Invalid nested @media and @container rules */`;
              });
            } else {
              singleMediaQueries.push({ mediaNode: atRule2, declNodes });
            }
          },
          // MARK: - - Container
          ...
          container(atRule2) {
            ...
            if (isNested && isSameAtRule) {
              ...
              singleContainerQueries.push({ mediaNode: atRule2, declNodes });
            }
          }
        },
        // MARK: OnceExit
        OnceExit() {
          ...
          noMediaQueries.forEach(({ ruleNode, declNodes }) => {
            declNodes.forEach((decl2) => {
              const args = extractTwoValidClampArgs(decl2.value);
              const [lower, upper] = args.map((val) => convertToRem(val, rootFontSize, spacingSize, customProperties));
              if (!args || !lower || !upper) {
                decl2.value = ` ${decl2.value} /* Invalid clamp() values */`;
                return;
              }
              ...
              ruleNode.append(postcss_default.decl({ prop: decl2.prop, value: clamp }));
              decl2.remove();
            });
          });
          singleMediaQueries.forEach(({ mediaNode, declNodes }) => {
            ...
            declNodes.forEach((decl2) => {
              ...
              if (mediaNode.params.includes(">")) {
                ...
                const maxScreen = screenValues[screenValues.length - 1];
                const singleMQ = postcss_default.atRule({ name: "media", params: `(width >= ${minScreen})` });
                const clamp = generateClamp(lower, upper, minScreen, maxScreen, rootFontSize, spacingSize);
                singleMQ.append(postcss_default.decl({ prop: decl2.prop, value: clamp }));
                newMediaQueries.push(singleMQ);
                decl2.remove();
              } else if (mediaNode.params.includes("<")) {
                const minScreen = screenValues[0];
                const maxScreen = mediaNode.params.match(/<([^)]+)/)[1].trim();
                const singleMQ = postcss_default.atRule({ name: "media", params: mediaNode.params });
                const clamp = generateClamp(lower, upper, minScreen, maxScreen, rootFontSize, spacingSize);
                singleMQ.append(postcss_default.decl({ prop: decl2.prop, value: clamp }));
                newMediaQueries.push(singleMQ);
                decl2.remove();
          ...
          singleContainerQueries.forEach(({ mediaNode, declNodes }) => {
            ...
            declNodes.forEach((decl2) => {
              ...
              if (mediaNode.params.includes(">")) {
                ...
                const singleCQ = postcss_default.atRule({ name: "container", params: `${containerName} (width >= ${minContainer})` });
                const clamp = generateClamp(lower, upper, minContainer, maxContainer, rootFontSize, spacingSize, true);
                singleCQ.append(postcss_default.decl({ prop: decl2.prop, value: clamp }));
                newContainerQueries.push(singleCQ);
                decl2.remove();
              } else if (mediaNode.params.includes("<")) {
                const minContainer = screenValues[0];
                const maxContainer = mediaNode.params.match(/<([^)]+)/)[1].trim();
                const singleCQ = postcss_default.atRule({ name: "container", params: `${containerName} ${mediaNode.params}` });
                const clamp = generateClamp(lower, upper, minContainer, maxContainer, rootFontSize, spacingSize, true);
                singleCQ.append(postcss_default.decl({ prop: decl2.prop, value: clamp }));
                newContainerQueries.push(singleCQ);
                decl2.remove();
          ...
          doubleNestedMediaQueries.forEach(({ parentNode, mediaNode, declNodes }) => {
            declNodes.forEach((decl2) => {
              ...
              decl2.value = generateClamp(lower, upper, minScreen, maxScreen, rootFontSize, spacingSize, false);
            });
          });
          doubleNestedContainerQueries.forEach(({ parentNode, mediaNode, declNodes }) => {
            declNodes.forEach((decl2) => {
              ...
              decl2.value = generateClamp(lower, upper, minContainer, maxContainer, rootFontSize, spacingSize, true);
            });
          });
        }
      };
...
//# sourceMappingURL=clampwind.esm.js.map
